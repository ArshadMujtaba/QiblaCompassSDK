...
/**
 * Calculates the distance from the device's location to the Kaaba (Qibla).
 */
fun calculateQiblaDistanceKm(location: Location?): Double? {
    if (location == null) return null

    val earthRadiusKm = 6371.0

    val dLat = (KAABA_LATITUDE - location.latitude).toRadians()
    val dLon = (KAABA_LONGITUDE - location.longitude).toRadians()

    val lat1 = location.latitude.toRadians()
